...
import { createMyTableService } from './mytable-service'
import { RECOMMENDED_PLUGINS } from './plugin-catalog'
import { runLegacyMigration } from './legacy-migrate'
import { WorktableSettingsSection, setSettingsT, settingsSectionLabel } from './SettingsSection'
...
const PERSIST_KEY = 'dsh.mytable.view.v1'
const PROJECTS_KEY = 'dsh.mytable.projects.v1'
const MIN_TOP = 56
const SNAP_PX = 32
...
/**
 * 新项目的固定布局（布局选择已下线）：
 *   - 内容窗一律在左侧（一个空窗格，打开后显示「新建窗口选择器」，用户再用 ⇥/⤓ 自己分栏）
 *   - 聊天框固定在右侧、占满高度（chatFullHeight），不再有顶排横跨聊天框上方的错位
 * 已有的旧布局（预设/存档）仍按自己的 spec 渲染，不受影响。
 */
function buildDefaultLayout(name: string): LayoutSpec {
  return {
    id: 'layout-' + Date.now().toString(36),
    title: name,
    left: null,
    top: null,
    main: [{ id: 'p1', title: '窗口1', min: 200, content: null }],
...
/** 兼容旧调用：无论给什么预设 id，都产出固定布局（内容窗在左、聊天框右侧满高）。 */
function buildLayout(_presetId: string, name: string): LayoutSpec {
  return buildDefaultLayout(name)
}

/** 布局缩略图（迷你窗格示意；聊天窗蓝色 💬） */
function presetThumb(defId: string) {
  const cell = (chat: boolean, key: string) => (
    <span key={key} className={'dsh-mt_thumbCell' + (chat ? ' dsh-mt_thumbChat' : '')}>{chat ? '💬' : ''}</span>
  )
  if (defId === '2h') {
    return <span className="dsh-mt_thumb"><span className="dsh-mt_thumbRow">{cell(false, 'a')}{cell(true, 'b')}</span></span>
  }
  if (defId === '3h') {
    return <span className="dsh-mt_thumb"><span className="dsh-mt_thumbRow">{cell(false, 'a')}{cell(false, 'b')}{cell(true, 'c')}</span></span>
  }
  if (defId === 'g4') {
    // 田字格：左侧 2×2 均分，右侧对话整列（5 视窗）
    return (
      <span className="dsh-mt_thumb dsh-mt_thumbCols">
        <span className="dsh-mt_thumbCol">
          <span className="dsh-mt_thumbRow">{cell(false, 'a')}{cell(false, 'b')}</span>
          <span className="dsh-mt_thumbRow">{cell(false, 'c')}{cell(false, 'd')}</span>
        </span>
        <span className="dsh-mt_thumbCol">{cell(true, 'e')}</span>
      </span>
    )
  }
  if (defId === 'l23') {
    ...
    return (
      <span className="dsh-mt_thumb dsh-mt_thumbCols">
        <span className="dsh-mt_thumbCol">
          <span className="dsh-mt_thumbRow">{cell(false, 'a')}{cell(false, 'b')}</span>
          <span className="dsh-mt_thumbRow">{cell(false, 'c')}{cell(false, 'd')}{cell(false, 'e')}</span>
        </span>
        <span className="dsh-mt_thumbCol">{cell(true, 'f')}</span>
      </span>
    )
  }
  if (defId === 'l13') {
    ...
    return (
      <span className="dsh-mt_thumb dsh-mt_thumbCols">
        <span className="dsh-mt_thumbCol">
          <span className="dsh-mt_thumbRow">{cell(false, 'a')}</span>
          <span className="dsh-mt_thumbRow">{cell(false, 'b')}{cell(false, 'c')}{cell(false, 'd')}</span>
        </span>
        <span className="dsh-mt_thumbCol">{cell(true, 'e')}</span>
      </span>
    )
  ...
  if (defId === 'grid') {
    return (
      <span className="dsh-mt_thumb">
        <span className="dsh-mt_thumbRow">{cell(false, 'a')}{cell(false, 'b')}</span>
        <span className="dsh-mt_thumbRow">{cell(false, 'c')}{cell(true, 'd')}</span>
      </span>
    )
  }
  if (defId === 'l2') {
    ...
    return (
      <span className="dsh-mt_thumb dsh-mt_thumbCols">
        <span className="dsh-mt_thumbCol">
          <span className="dsh-mt_thumbRow">{cell(false, 'a')}</span>
          <span className="dsh-mt_thumbRow">{cell(false, 'b')}</span>
        </span>
        <span className="dsh-mt_thumbCol">{cell(true, 'c')}</span>
      </span>
    )
  }
  return (
    <span className="dsh-mt_thumb dsh-mt_thumbCols">
      <span className="dsh-mt_thumbCol">
        <span className="dsh-mt_thumbRow">{cell(false, 'a')}</span>
        <span className="dsh-mt_thumbRow">{cell(false, 'b')}{cell(false, 'c')}</span>
      </span>
      <span className="dsh-mt_thumbCol">{cell(true, 'd')}</span>
    </span>
  )
}
...
const KNOWLEDGE_PACK = [
  ...
  '- 改完插件在 dsh-mytable 包根目录执行 npm run build；重启 dsh --profile mytable 或浏览器 F5 生效。',
...
function loadNotifyAck(): Record<string, string> {
  try {
    const raw = localStorage.getItem('dsh.mytable.notifyAck.v1')
...
function saveNotifyAck(sid: string, state: string) {
  try {
    ...
    ack[sid] = state
    localStorage.setItem('dsh.mytable.notifyAck.v1', JSON.stringify(ack))
  } catch {}
}
...
const pendingMountRef: { current: Record<string, any> } = { current: (() => {
  try { return JSON.parse(localStorage.getItem('dsh.mytable.pendingMount.v1') ?? '{}') ?? {} } catch { return {} }
})() }
...
const mountedWidgetRef: { current: Record<string, string> } = { current: (() => {
  try { return JSON.parse(localStorage.getItem('dsh.mytable.mountedWidget.v1') ?? '{}') ?? {} } catch { return {} }
})() }
function recordMountedWidget(projectId: string, raw: string) {
  if (!projectId || !raw) return
  mountedWidgetRef.current[projectId] = raw
  try { localStorage.setItem('dsh.mytable.mountedWidget.v1', JSON.stringify(mountedWidgetRef.current)) } catch {}
}
...
function buildMountContent(folder: string, d: any): any {
  ...
  // 「设置 → 工作台 → 网页预览」关闭时不挂站点 iframe，退化为文件预览（纯文本源码）
  if (!previewEnabled('html')) return { kind: 'file', path: full }
...
function clearNotifyAck(sid: string) {
  try {
    ...
    if (all[sid]) {
      delete all[sid]
      localStorage.setItem('dsh.mytable.notifyAck.v1', JSON.stringify(all))
    }
  } catch {}
}
...
function WorktableSection(props: any) {
  ...
  const [updateCheckOn, setUpdateCheckOn] = useState<boolean>(() => localStorage.getItem('dsh.mytable.updateCheck.v1') !== '0')
  ...
  const checkUpdates = useCallback(async (force = false) => {
    ...
    const last = Number(localStorage.getItem('dsh.mytable.lastUpdateCheck.v1') ?? '0')
    ...
    try {
      ...
      localStorage.setItem('dsh.mytable.lastUpdateCheck.v1', String(Date.now()))
      const tag = (d.tag_name ?? '').replace(/^v/, '')
      if (!tag || cmpVer(tag, LOCAL_VERSION) <= 0) { setUpdateStatus('uptodate'); return }
      if ((localStorage.getItem('dsh.mytable.skipVersion.v1') ?? '') === tag) { setUpdateStatus('uptodate'); return }
  ...
  const skipUpdate = () => {
    if (updateInfo) {
      localStorage.setItem('dsh.mytable.skipVersion.v1', updateInfo.latest)
      setUpdateInfo(null)
    }
  }
  const toggleUpdateCheck = () => {
    setUpdateCheckOn((v) => {
      localStorage.setItem('dsh.mytable.updateCheck.v1', v ? '0' : '1')
      return !v
    })
  }
  ...
  useEffect(() => {
    const env = {
      ...
      getSessionList: () => sessionsSnapshotStore.snapshot,
      custom: {
        getProjects: () => {
      ...
      console: {
        ...
        getPhotoLib: async () => {
          ...
          if (records.length === 0 && localStorage.getItem('dsh.mytable.defaultBgSeeded.v1') !== '1') {
            try {
              await photoStore.add(new Blob([DEFAULT_BG_SVG], { type: 'image/svg+xml' }))
              await photoStore.add(b64ToBlob(WAVE_BG_B64, 'image/jpeg'))
              localStorage.setItem('dsh.mytable.defaultBgSeeded.v1', '1')
        ...
        addPhoto: async (blob: Blob) => {
          ...
          try { localStorage.removeItem('dsh.mytable.consoleBgPhoto.v1') } catch {}
    ...
    setSplitEnv(env)
    // @ 引用（资源管理器 / 预览头）用的宿主服务 + 会话作用域：与分栏引擎共用同一个 getScope
    setReferenceServices({ conversation: sessionBridge?.conversation ?? null, sessions: sessionBridge?.sessions ?? null, getScope: env.getScope })
...
function buildCustomLayoutPrompt(req: string): string {
  return [
    ...
    '6. 在 presetThumb(defId) 里为新 id 增加一个缩略图分支（用 dsh-mt_thumb / dsh-mt_thumbCol / dsh-mt_thumbRow / cell()；聊天窗用 cell(true) 的蓝色 💬 格）。',
  ...
  const openSplit = useCallback((spec: LayoutSpec) => {
    ...
    if (splitStore.active && splitStore.spec?.id === spec.id) {
      ...
      if (pending) {
        ...
        try { localStorage.setItem('dsh.mytable.pendingMount.v1', JSON.stringify(pendingMountRef.current)) } catch {}
      }
    }
  }, [projects.views])
  ...
  // 1) 位置映射：给每张卡片标 data-mt-id，图标覆盖写到卡片第一个子元素（通用结构约定）；
  // 2) 委托点击（捕获阶段）：卡片图标 → 图标选择器；有视图覆盖的项目 → 用引擎打开该视图。
  useEffect(() => {
    const sync = () => {
      const box = document.querySelector<HTMLElement>('.dsh-mt_projects')
      if (!box) return
      // 子座位把所有卡片包在一个无类容器里：直接取卡片按钮（DOM 序 = 注册序），排除自渲染的布局卡
      const cards = Array.from(box.querySelectorAll<HTMLElement>('button:not(.dsh-mt_layout)'))
      cards.forEach((el, i) => {
        const id = aliveRegisteredIds[i]
        if (id) {
          el.setAttribute('data-mt-id', id)
          // 对话绑定按钮：追加为最后一个子元素（不破坏 children[0] 图标约定），中间偏右
          let bindBtn = el.querySelector<HTMLElement>('.dsh-mt_bindBtn')
          if (!bindBtn) {
          ...
          bindBtn.setAttribute('data-mt-bind', id)
          ...
          if (arrow) (arrow as HTMLElement).classList.add('dsh-mt_resArrow')
          const tip = bound
  ...
  useEffect(() => {
    const onDocClick = (e: MouseEvent) => {
      ...
      const card = target && target.closest
        ? (target.closest('.dsh-mt_projects [data-mt-id]') as HTMLElement | null)
        : null
      if (!card) return
      const pid = card.getAttribute('data-mt-id')
      if (!pid) return
      // 绑定按钮点击 → 打开对话绑定弹窗（阻止卡片自身打开项目）
      const bindBtn = (target && target.closest ? target.closest('.dsh-mt_bindBtn') : null) as HTMLElement | null
      if (bindBtn) {
        e.stopPropagation()
        e.preventDefault()
    ...
    <div ref={rootRef} className={'dsh-mt_section' + (isFloat ? ' dsh-mt_float' : '')} style={isFloat ? floatStyle : dockedStyle}>
      ...
      {view.searchOpen && (
        <div className="dsh-mt_search">
          ...
          />
          <button type="button" className="dsh-mt_searchClear" aria-label={t('search.close')}
      ...
      {viewOptionsOpen && (
        <div ref={settingsRef} className="dsh-mt_manage dsh-mt_pop dsh-mt_settings" style={{ position: 'fixed', left: popLeft, top: settingsTop ?? popTop, width: 280, zIndex: 80 }}>
          ...
          >✕</button>
          {updateInfo && (
            <div className="dsh-mt_updateCard">
              <div className="dsh-mt_updateHead"><span className="dsh-mt_updateDot" />{t('update.available')} · v{updateInfo.latest}</div>
              <div className="dsh-mt_updateVers">{t('update.current')} v{LOCAL_VERSION} → v{updateInfo.latest}</div>
              {updateInfo.notes && <div className="dsh-mt_updateNotes">{updateInfo.notes}</div>}
              <div className="dsh-mt_updateCmd">{UPGRADE_CMD}</div>
              <div className="dsh-mt_updateBtns">
                <button type="button" className="dsh-mt_updateBtn dsh-mt_updateBtnCopy" onClick={() => void copyUpgradeAi()}>
                  {updateCopied ? '✓ ' + t('update.copied') : <>{ICON_SPARK} {t('update.copyAi')}</>}
                </button>
                <button type="button" className="dsh-mt_updateBtn" onClick={skipUpdate}>{t('update.skip')}</button>
              </div>
              <div className="dsh-mt_updateHint">{t('update.upgradeHint')}</div>
            </div>
          )}
          <div className="dsh-mt_manageHead">
            <span className="dsh-mt_manageTitle">{t('sort.label')}</span>
          </div>
          <div className="dsh-mt_sortRow">
            <button type="button" className="dsh-mt_sortBtn" data-on={view.orderBy === 'manual'}
              onClick={() => persistView({ orderBy: 'manual' })}>{t('sort.manual')}</button>
            <button type="button" className="dsh-mt_sortBtn" data-on={view.orderBy === 'recent'}
              onClick={() => persistView({ orderBy: 'recent' })}>{t('sort.recent')}</button>
          </div>
          <div className="dsh-mt_menuSep" />
          <div className="dsh-mt_manageHead">
            <span className="dsh-mt_manageTitle">{t('manage.title')}</span>
          </div>
          {effectiveOrder.map((id) => {
            ...
            return (
              <div
                key={id}
                className={'dsh-mt_manageRow' + (isHidden ? ' dsh-mt_manageRowOff' : '')}
                draggable
          ...
          <div className="dsh-mt_versionRow">
            <span>
              v{LOCAL_VERSION}
              {updateStatus === 'uptodate' && !updateInfo ? ' · ' + t('update.upToDate') : ''}
              {updateStatus === 'failed' && !updateInfo ? ' · ' + t('update.checkFail') : ''}
            </span>
            <span className="dsh-mt_versionActions">
              <button type="button" className="dsh-mt_updateBtn" disabled={updateStatus === 'checking'} onClick={() => void checkUpdates(true)}>
                {updateStatus === 'checking' ? t('update.checking') : t('update.checkNow')}
              </button>
              <span className="dsh-mt_updateToggle" onClick={toggleUpdateCheck}>
        ...
        <div className="dsh-mt_menu dsh-mt_pop dsh-mt_bindPop" style={{ position: 'fixed', left: bindPick.x, top: bindPick.y, width: 280, zIndex: 84 }}>
          ...
          <div className="dsh-mt_bindFolderBox">
            <div className="dsh-mt_bindFolderRow">
              <span className="dsh-mt_bindFolderLabel">📁 {t('bind.folder')}</span>
              <button type="button" className="dsh-mt_bindFolderChange" disabled={pickBusy} onClick={changeBindFolder}>
            ...
            {pickBusy && <p className="dsh-mt_folderPickHint">{t('add.folderPickerHint')}</p>}
            <div className={'dsh-mt_bindFolderPath' + (projects.folders[bindPick.id] ? '' : ' dsh-mt_bindFolderPathNone')} title={projects.folders[bindPick.id] ?? ''}>
            ...
            {manualPathFor === 'bind' && (
              <div className="dsh-mt_addFolderRow">
                ...
                <button type="button" className="dsh-mt_bindFolderChange" onClick={() => applyManualPath('bind')}>{t('add.folderManualOk')}</button>
              </div>
            )}
            {pickErr.bind && <p className="dsh-mt_addError">{pickErr.bind}</p>}
          </div>
          {/* 绑定对话框（与项目文件夹同格式）：第一行 💬+标题（最右解绑）；第二行 分组 | 对话名（点击右侧弹列表，再点反选收起）；框内下方：说明 */}
          <div className="dsh-mt_bindFolderBox">
            <div className="dsh-mt_bindFolderRow">
              <span className="dsh-mt_bindFolderLabel">💬 {t('bind.title')}</span>
              {projects.bindings[bindPick.id] && (
                <button type="button" className="dsh-mt_bindUnbind" onClick={() => setProjectBinding(bindPick.id, null)}>{t('bind.unbind')} ✕</button>
              )}
            </div>
            {(() => {
              ...
              if (!sid) {
                return (
                  <button type="button" className="dsh-mt_bindConvRow dsh-mt_bindConvRowNone" title={t('bind.tipUnbound')} onClick={() => setBindListOpen((v) => !v)}>
                    <span className="dsh-mt_bindNoneText">{t('bind.unbound')} · {t('bind.clickPick')}</span>
                  </button>
                )
              }
              const info = bindInfoOf(bindGroups, sid)
              return (
                <button type="button" className="dsh-mt_bindConvRow" title={t('bind.tipBound', { name: info.title })} onClick={() => setBindListOpen((v) => !v)}>
                  <span className="dsh-mt_bindFolder" title={info.folder}>📂 {info.folder}</span>
                  <span className="dsh-mt_bindSep" aria-hidden>|</span>
                  <span className="dsh-mt_bindConvName" title={info.title}>{info.title}</span>
                  <span className="dsh-mt_bindConvChevron" aria-hidden>▾</span>
                </button>
              )
            })()}
            <p className="dsh-mt_bindHint">{t('bind.hint')}</p>
          </div>
        </div>
      )}
      ...
      {bindPick && bindListOpen && (() => {
        ...
        return (
        <div className="dsh-mt_menu dsh-mt_pop dsh-mt_bindListPop" style={{ position: 'fixed', left: listLeft, top: clamp(bindPick.y, 8, window.innerHeight - 348), width: 260, zIndex: 86 }}>
          <div className="dsh-mt_selectList dsh-mt_bindList">
            {bindGroups.map((g, gi) => (
              <Fragment key={g.title || 'g' + gi}>
                ...
                {g.sessions.map((s) => (
                  <button
                    key={s.id}
                    type="button"
                    className={'dsh-mt_selectItem' + (projects.bindings[bindPick.id] === s.id ? ' dsh-mt_selectItemOn' : '')}
                    onClick={() => { setProjectBinding(bindPick.id, s.id); setBindListOpen(false) }}
                  >
                    <span className="dsh-mt_selectItemTitle">{s.title}</span>
                    {s.isCurrent && <span className="dsh-mt_selectCurrent">{t('custom.sessionCurrent')}</span>}
                  </button>
                ))}
              </Fragment>
            ))}
      ...
      {consoleBind && <div className="dsh-mt_popBackdrop" style={{ zIndex: 85 }} onClick={() => setConsoleBind(null)} />}
      {consoleBind && (
        <div className="dsh-mt_menu dsh-mt_pop dsh-mt_consoleBindPop" style={{ position: 'fixed', left: consoleBind.x, top: consoleBind.y, width: 560, zIndex: 86 }}>
          <div className="dsh-mt_consoleBindCols">
            <div className="dsh-mt_consoleBindCol">
              <span className="dsh-mt_consoleBindLabel">➕ {t('console.joinExisting')}</span>
              <div className="dsh-mt_selectList dsh-mt_consoleBindList">
                  ...
                  <Fragment key={g.title || 'g' + gi}>
                    ...
                    {g.sessions.map((s) => (
                      <button
                        key={s.id}
                        type="button"
                        className={'dsh-mt_selectItem' + (projects.bindings[CONSOLE_ID] === s.id ? ' dsh-mt_selectItemOn' : '')}
                ...
                {consoleGroups.length === 0 && <div className="dsh-mt_consoleBindEmpty">{t('console.noSessions')}</div>}
              </div>
            </div>
            <div className="dsh-mt_consoleBindCol dsh-mt_consoleBindColNew">
              <span className="dsh-mt_consoleBindLabel">✨ {t('console.newConv')}</span>
              <select className="dsh-mt_consoleSelect" value={consoleMode} onChange={(e) => setConsoleMode(e.target.value as 'none' | 'existing' | 'new')}>
              ...
              {consoleMode === 'existing' && (
                <select className="dsh-mt_consoleSelect" value={consoleWsId} onChange={(e) => setConsoleWsId(e.target.value)}>
                  {listWorkspaces().map((w) => <option key={w.id} value={w.id}>{w.title}</option>)}
                </select>
              )}
              {consoleMode === 'new' && (
                <>
                  <input className="dsh-mt_consoleInput" placeholder={t('console.newParentPh')} value={consoleParent} onChange={(e) => setConsoleParent(e.target.value)} />
                  <input className="dsh-mt_consoleInput" placeholder={t('console.newNamePh')} value={consoleName} onChange={(e) => setConsoleName(e.target.value)} />
                </>
              )}
              {consoleErr && <p className="dsh-mt_consoleErr">{t('console.bindFail')}</p>}
              <button type="button" className="dsh-mt_consoleCreateBtn" disabled={consoleBusy} onClick={bindConsoleNew}>
      ...
      {customOpen && <div className="dsh-mt_popBackdrop" style={{ zIndex: 83 }} onClick={() => setCustomOpen(false)} />}
      {customOpen && (
        <div className="dsh-mt_menu dsh-mt_pop" style={{ position: 'fixed', left: popLeft, top: popTop, width: 340, zIndex: 84 }}>
          <span className="dsh-mt_menuLabel">✨ {t('customLayout.title')}</span>
          <textarea
            className="dsh-mt_customLayoutInput"
            autoFocus
            rows={6}
          ...
          />
          <button type="button" className="dsh-mt_customLayoutBtn" onClick={copyCustomLayout}>{t('customLayout.copy')}</button>
          {copiedToast && (
            <p className={'dsh-mt_customLayoutToast' + (copiedToast === 'fail' ? ' dsh-mt_customLayoutToastFail' : '')}>
      ...
      {requestDelete && <div className="dsh-mt_confirmBackdrop" onClick={() => setRequestDelete(null)} />}
      {requestDelete && (
        <div className="dsh-mt_confirm" role="alertdialog">
          <div className="dsh-mt_confirmTitle">⚠️ {t('confirm.title')}</div>
          <div className="dsh-mt_confirmBody">
          ...
          <div className="dsh-mt_confirmActions">
            <button type="button" className="dsh-mt_confirmCancel" onClick={() => setRequestDelete(null)}>{t('confirm.cancel')}</button>
            <button type="button" className="dsh-mt_confirmDelete" onClick={doDelete}>{t('confirm.delete')}</button>
          </div>
        </div>
      )}
      ...
      <div className="dsh-mt_projects" data-managing={viewOptionsOpen ? 'true' : undefined}>
        ...
        {renderProjectSlot
          ...
          : <div className="dsh-mt_empty">{t('empty')}</div>}
        {visibleLayouts.map((l) => (
          <button
            key={l.id}
            type="button"
            className="dsh-mt_layout"
      ...
      {visibleShortcuts.length > 0 && (
        <div className="dsh-mt_shortcuts">
          {visibleShortcuts.map((s) => (
            <a
              key={s.id}
              className="dsh-mt_shortcut"
              href={s.href}
              target="_blank"
            ...
            >
              <span
                className="dsh-mt_shortcutIcon dsh-mt_iconPick"
                role="button"
                tabIndex={0}
                title={t('icons.change')}
...
export function apply(ctx: any) {
  ...
  // 必须早于任何状态读取（loadView/loadProjects/loadPrefs 都在组件挂载时读），
  // 所以放在 apply 的第一行；内部幂等、失败静默。
  try { runLegacyMigration() } catch {}
  ...
  ctx.effect(() => {
    ...
    style.setAttribute('data-dsh-plugin', 'dsh-mytable')
  ...
  syncIds()
  ctx.effect(() => disposeSubscribe, 'dsh-mytable: project registry watch')
  ...
  if (sessionsList && typeof sessionsList.getSnapshot === 'function') {
    ...
    ctx.effect(() => disposeScope, 'dsh-mytable: session scope watch')
  }

  // 聊天中的文件卡片默认会调用 sidebarRight.openResource，从而展开宿主右侧栏。
  // 工作区已打开时接管同会话文件，把它作为主窗标签页打开；其它资源继续走宿主原逻辑。
  try {
    const fileRoutingSeat = ctx.inject(['sidebarRight'], (injected: any) => {
      const sidebarRight = injected.get('sidebarRight') as any
      if (!sidebarRight || typeof sidebarRight.openResource !== 'function') return
      const original = sidebarRight.openResource
      const routedOpenResource = function (this: any, address: string, ...args: any[]) {
        const snapshot = sessionScopeStore.snapshot
        const scope = snapshot?.sessionId
          ? { sessionId: String(snapshot.sessionId), cwd: String(snapshot.cwd ?? '') }
          : null
        if (typeof address === 'string' && splitStore.openFileResource(address, scope)) return
        return original.call(this, address, ...args)
      }
      try {
        sidebarRight.openResource = routedOpenResource
      } catch {
        return
      }
      return () => {
        try {
          if (sidebarRight.openResource === routedOpenResource) sidebarRight.openResource = original
        } catch {}
      }
    })
  ...
  // id/order/label；等待槽声明用 slots.inject（声明未到就注册会静默丢失且不重试）。
  // 文案走 ctx.locale.bind('worktable') 的稳定翻译函数（宿主没装 locale 服务时回退 zh 词典）。
  try {
    setSettingsT(ctx.locale && typeof ctx.locale.bind === 'function' ? ctx.locale.bind(NS) : null)
  } catch {
    setSettingsT(null)
  }
  ...
  }, WorktableSettingsSection), 'dsh-mytable: settings section')

  // 对外开放接口：其它插件 `inject: ['mytable']` 后即可
  //   ctx.mytable.registerPaneType({ id, title, icon, url|mount })   → 新增窗口类型
  //   ctx.mytable.registerFileViewer({ id, title, exts, mount|mode }) → 新增文件预览器
  // 注册项自动进「设置 → 工作台」两组清单（可开关），并在选择器 / 预览分发里实时生效。
  try {
    ...
    // 自测钩子：把 @ 引用这条链路摊开（作用域 / 落点 / 草稿原文都能一眼看到）
    try {
      ;(window as any).__dshMytableRef = {
        currentScope,
        referenceInChat,
        /** 当前会话草稿原文（宿主把草稿放在 conversation 服务里；没有会话/服务时返回 null） */
        readDraft: (): string | null => {
          try {
            const bridge = sessionBridge as any
            const scope = currentScope()
            const actx = scope && bridge?.sessions?.scope ? bridge.sessions.scope(scope.sessionId) : undefined
            if (!actx || !bridge?.conversation?.input) return null
            return String(bridge.conversation.input.for(actx).state.getSnapshot().draft ?? '')
          } catch { return null }
        },
        clearDraft: (): boolean => {
          try {
            ...
            const scope = currentScope()
            const actx = scope && bridge?.sessions?.scope ? bridge.sessions.scope(scope.sessionId) : undefined
            if (!actx || !bridge?.conversation?.input) return false
            bridge.conversation.input.for(actx).setDraft('')
            return true
          } catch { return false }
        },
      }
    ...
    try { ctx.logger?.warn?.('[dsh-mytable] ctx.mytable 服务注册失败：' + String(error)) } catch {}
  }
}
