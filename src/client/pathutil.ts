...
/**
 * 把路径写成相对 base 的**正斜杠**相对写法（`@` 引用与树里的相对显示都用它）。
 * 大小写不敏感（Windows 习惯）；base 为空、或路径不在 base 之下时，退化为去掉开头分隔符的原串。
 */
export function relativeTo(base: string, p: string): string {
  const norm = (s: string): string => s.replace(/\\/g, '/').replace(/\/+$/, '')
  const b = norm(base)
  const t = norm(p)
  if (!b) return t.replace(/^\/+/, '')
  const bl = b.toLowerCase()
  const tl = t.toLowerCase()
  if (tl === bl) return '.'
  if (tl.startsWith(bl + '/')) return t.slice(b.length + 1)
  return t.replace(/^\/+/, '')
}
