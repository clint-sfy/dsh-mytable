...
if (pluginManifest.version !== pkg.version) {
  throw new Error('dsh-mytable build: package.json version (' + pkg.version + ') != dsh.plugin.json version (' + pluginManifest.version + ') — 发版前先统一两处版本号')
}

const clientBanner = {
  // ModuleLoader id 必须 = 包名：宿主 client-modules 以入口解析到的 package.json name
  // 作为模块 id，bundle 若以别的 id 注册会在启动时报 "loaded without registering"。
  js: "window.__ModuleLoader__.load({ id: 'dsh-mytable', factory: (require) => { var module = { exports: {} }; var exports = module.exports;",
}
...
console.log('[dsh-mytable build] done: lib/index.js, lib/client.js')
